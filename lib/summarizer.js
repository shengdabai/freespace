...
const COMMIT_PATTERN = /^(feat|fix|docs|style|refactor|perf|test|build|ci|chore|revert)(\(.+\))?:\s*/;
...
export function extractKeyword(title) {
  ...
  // Detect commit format "feat: ..." → keep as-is (already clean)
  if (COMMIT_PATTERN.test(result)) {
